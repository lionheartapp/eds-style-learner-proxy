...
async function synthesize(analyses) {
  const clusters = {};
...
# 1. Core Color System
Exact hex values. Note: Edmunds primary is navy blue and orange accent.

# 2. Typography System
Note: Edmunds primary font is Helvetica Neue. Document the full type scale.

# 3. Spacing & Layout
Grid, spacing scale, padding values, max-width, breakpoints.
...
async function runPipeline(options = {}) {
  ...
  try {
      ...
      try {
        ...
        for (const proj of (pd.projects || [])) {
          ...
          for (const f of (fd.files || [])) {
            if (filterRx.test(f.name)) allFiles.push({ key: f.key, name: f.name, teamLabel: team.label });
          }
          await sleep(200);
        }
    ...
    for (const [fk, ff] of Object.entries(byFile)) {
      for (let i = 0; i < ff.length; i += 50) {
        const batch = ff.slice(i, i+50);
        const ids = batch.map(f => f.id).join(',');
        try {
          const img = await figmaGet(`/images/${fk}?ids=${encodeURIComponent(ids)}&scale=1&format=png`);
    ...
    const styleGuide = await synthesize(analyses);
...
app.get('/api/supabase-data', async (req, res) => {
  ...
  try {
    const [aRes, sgRes] = await Promise.all([
    ...
    res.json({ analyses: Array.isArray(analyses) ? analyses : [], styleGuide: guides[0]?.content || '' });
  } catch(e) {
